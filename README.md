...
The unreleased D-079 joint-affine substitution returns right reward and successor after substituting the left successor; it admits but does not accumulate left reward. Complete all-shared or all-fresh right-owner requests are checked against both full zero-inclusive declaration manifests. Gate B explicitly supersedes Gate A's retained-only renaming with declaration-complete membership, collisions, scope coverage, and work. Results are sealed and non-chainable; materialization and extrema retain historical rational-bit admission even after cancellation. D-079 remains `Proposed`, with independent implementation review and acceptance separate.
