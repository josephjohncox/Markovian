...
Raw or cyclic `OpenSystem` values cannot use this interpreter. The separate `Markovian.Feedback.*.Exact` modules support only explicit one-tick delay, checked proper first-exit coproduct routing, nilpotent reward-and-duration-preserving closure, and accepted but unreleased strict-discount affine value coefficients. They do not accept cyclic `OpenSystem` topology. Universal trace, arbitrary fixed points, arbitrary hypergraph black-boxing, continuous-time open Markov processes, and unrestricted MDP black-boxing remain deferred.

Checked feedback keeps normalized routing separate from raw matrix trace. Proper first exit solves and validates `H = C + D H`. Delayed execution retains output and successor state jointly. Timed execution retains reward, duration, and output jointly. Strict-discount value feedback exposes only opaque nominal `A` and `K` coefficients after a bounded exact solve and four literal equation checks; it exposes no evaluator or normalized exit channel. A half-loop and half-exit has a valid marginal first exit. Timed feedback rejects it because duration support is unbounded. All operations have explicit limits. Deterministic reports include exact work and rational maxima for discarded and retained values. D-078 is `Accepted` only for these original coefficients and four exact equations; it remains unreleased. EL-04's separately reviewed event-reward JVP is not accepted by that decision.
...
The tensor package is host-only and F64-only. Its region indices and nominal roles reject direct fixed-index tensor escape and coercion, not existential packages or captured IO actions. All region-dependent reads, refinements, primitives and allocations must execute and complete inside the live callback. The caller must join or cancel-and-join dependent children before every exit, including Left, exceptions and asynchronous interruption; the runner does not join them. Returning ordinary copies whose reads completed inside the callback is supported, but later observation through escaped actions or retained tensors is not. Unsupported post-close use is not uniformly rejected, and observers do not synchronize with close. Closing drains the committed registry and attempts finalization, including asynchronous retries; neither logical cleanup nor default GC-pinned retention promises prompt physical reclamation or supports post-finalization use. No current-default freed-buffer/UAF execution has been established. Rank zero is one scalar. Numerical primitives require finite values and use fixed-order single-threaded loops. Payload and work budgets are preflighted, including atomic multi-output VJPs. It has no arbitrary strides, broadcasting, mutation, raw pointers, BLAS, device buffers, generic reverse-program lowering, or performance claim. The separate SafeTensors package supports only bounded metadata-free F64 files; it rejects unsupported dtypes and metadata and does not serialize ownership or execution resources. The optional GPU package consumes checked tensor inputs only for positive-size F64 matrix multiplication and its declared VJP. D-067 and D-073 are accepted only for these bounded scopes.

The unreleased D-079 joint-affine substitution returns right reward and successor after substituting the left successor; it admits but does not accumulate left reward. Complete all-shared or all-fresh right-owner requests are checked against both full zero-inclusive declaration manifests. Gate B explicitly supersedes Gate A's retained-only renaming with declaration-complete membership, collisions, scope coverage, and work. Results are sealed and non-chainable; materialization and extrema retain historical rational-bit admission even after cancellation. D-079 is `Accepted` within its bounded, unreleased scope, by the [separate parent-authorized acceptance record](docs/DECISIONS.md#d-079-add-exact-joint-affine-continuous-kernels). D-080 is also accepted only for closed exact-polynomial quotation and frozen cumulative compilation; neither acceptance releases anything or accepts EL-03/EL-04/EL-05 or D-081–D-085.
...
[The tensor-fragment record](docs/evidence/CUDA-TENSOR-2026-09-02.md) retains focused differentials and raw 64-by-64 samples. It lacks a complete immutable same-session receipt and is historical narrative under D-077. [The 2026-08-26 record](docs/evidence/CUDA-2026-08-26.md) is historical evidence for the removed list API. Neither record supports a performance or device-correctness claim.
