...
- [ ] **R5 (`NEXT`) Review the physical D-081 repository design freeze.** Parent approved the [exact full v3 contract](docs/plans/D081-AFFINE-VIEWS.md) after independent prospective-design PASS `5ba479ba-964e-4da4-8f95-51831227309b` and the 63-file hash audit. The design is frozen; D-081 remains Proposed. Validate and independently review this documentation/lifetime-qualification delta before parent byte audit/commit, then obtain separate runtime implementation authorization. Historical transpose-only evidence is not a signed-map or base-pullback implementation PASS. Keep general dtype, mutation, broadcast pullback, and persistent devices excluded.
...
Existing evidence: [transpose reverse-equivalence repair](docs/evidence/D081-TRANSPOSE-REVERSE-EQUIVALENCE.md), not a wider affine-view implementation. The [full v3 normative plan](docs/plans/D081-AFFINE-VIEWS.md) records parent design approval under `/tmp/d081-repository-freeze-authority.md`; external model/typecheck/source evidence remains distinct from future runtime tests.
...
- [ ] Complete focused validation and independent review of the physical repository freeze; parent then audits bytes and commits. Current external validation inventory: `/tmp/d081-repository-freeze-61823aec/`.
- [ ] Obtain separate parent runtime authorization, then implement only the exact reviewed contract, including all legacy/new registry-spine and protected real-put publication paths. Preserve immutable storage and distinct owner/storage/region evidence.
