...
The user authorized this program after [the 2026-09-05 review](docs/evidence/LEARNING-REVIEW-2026-09-05.md). Follow its checkboxes and execution log. Freeze and review each semantic contract before implementation. The program's historical status records remain unchanged. Separate 2026-09-08 records accept D-077/original D-078 and subsequently the bounded, unreleased D-079/D-080 scopes. EL-03/EL-04/EL-05 and D-081 through D-085 remain `Proposed`; released history stays immutable.

## Post-release roadmap

This roadmap schedules governance review before acceptance. D-077, original D-078, and separately D-079/D-080 are `Accepted` for the bounded, unreleased scopes recorded below; D-081 through D-085 remain `Proposed`. New implementation work is `BLOCKED` until the applicable decision freezes exact exported signatures, failure precedence, package topology, ownership, limits, and evidence. Existing proposal-stage implementation evidence remains unaccepted until it is reviewed against that frozen contract.
...
- [ ] **R5 (`NEXT`) Obtain separate D-081 runtime-continuation authority.** Parent has adopted the [reviewed materialization addendum](docs/plans/D081-MATERIALIZATION-ADDENDUM.md), alongside the unchanged [canonical31bf plan](docs/plans/D081-AFFINE-VIEWS.md), after DESIGN PASS `8992df58-e052-4b23-9fd9-345febebf9d2`, documentation PASS `7f1a1fa6-ec54-4b26-a642-e33011ab9b74` and the pre-closeout audit. The [freeze record](docs/evidence/D081-MATERIALIZATION/FREEZE-RECORD.md) binds the adoption. It supersedes only reviewed resource/producer/failure/fixture seams and consolidates five clarifications. Final status-closeout review and parent byte audit/staging/commit remain; no Git commit or integration is claimed. D-081 remains Proposed; runtime continuation is held and scratch-r4 is not applied. Historical transpose-only and scratch design evidence are not whole-runtime PASS. Keep general dtype, mutation, broadcast pullback, and persistent devices excluded.
...
Post-release roadmap status: R0–R4 bounded technical review and parent acceptance authorization are complete. D-077/D-078 status-edit review `973f051f-ae7f-4bfd-8e79-2a1711a96670` and parent commit `3bfd8de50d64ea7d38008e5c5e1d3c6b595ff801` remain historical. D-079/D-080 acceptance is recorded in this separate status delta. Independent 13-file status-delta review `57601418` passed with P2; the changelog clarifications address that note. Final review `11ddbb94` passed and parent committed D079/D080 acceptance at `78669c3613302249c499eba99a959f75c4c59edc`. The r4 contract freeze, read with canonical31bf, is adopted after design/documentation PASS and the pre-closeout audit. `NEXT` is separate parent runtime-continuation authority, not implementation activity or decision acceptance; the closeout record still requires final review and parent byte audit/staging/commit. D-081–D-085 and EL-03/EL-04/EL-05 remain Proposed; D-082 remains `BLOCKED` on D-081. No new device feature or external action is authorized.
...
- [x] Implement only the separately authorized reviewed contract, including the explicitly FUTURE supersession of retained-only Gate A renaming semantics by declaration-complete membership, collisions, scope coverage, and work. Preserve public Gate A signatures, real-Borel outputs, nominal ownership, and historical records.
- [x] Run all plan fixtures: independent symbolic/corner oracles, shared/fresh joint moments, lost left reward, zero-row renaming/collisions, private manifest/witness invariants, positive clients and named negative boundary diagnostics, exact/one-below limits, cancellation, projection historical bits, and insufficient-work precedence. The authorized delta from `a8fe21e12b2d036e8ec22f10e25c4d07a5374f6f` passed the GHC 9.8.4 `-Werror` continuous suite and boundary script; logs are `/tmp/d079-implementation-worker/continuous-Werror.log` and `boundary.log`. Private mutation controls reject erased zero declarations, dropped original-name reservations, and substituted captured coordinates; constructor probes compile only in deliberately widened temporary export controls (`mutation-controls.log`). These are local implementation results, not independent review or acceptance.
- [x] Obtain fresh independent review of Gate A and the completed Gate B implementation with current evidence. Full semantic/delta review `02f71a3f-eef5-4d9b-bff6-b4cd0b092712` and corrective readiness PASS `e4bfb40c-1cd5-4389-b986-e2c3a18e8e24` cover the final 24-file delta. Both supported compilers and warning-enabled package-log Haddock checks passed; complete evidence is in `/tmp/d079-correction-validation-NnuNuzNF`. Parent independently rechecked all current file hashes/modes and reran the GHC 9.8.4 continuous suite before committing. This is not acceptance or release readiness.
- [x] Parent authorizes and records only bounded, unreleased Gate A plus frozen Gate B acceptance in this separate status edit, following full decision-specific PASS `b4c30c96-f3ef-40ba-8ff0-bdf038e9e4df` at `12f3794`. Gate A alone did not complete D-079. See [acceptance evidence](docs/DECISIONS.md#d-079d-080-acceptance-evidence-and-status-edit-boundary) for current-unit versus carried-forward archive/mutation attribution.
- [x] Obtain separate independent status-delta review: `57601418` reviewed the 13-file delta and returned PASS with P2. The changelog clarifications address P2; this review is distinct from the technical audits.
- [x] Parent committed the status delta at `78669c3613302249c499eba99a959f75c4c59edc` after final independent review PASS `11ddbb94` and follow-up validation.

#### R4 — D-080 first-order quotation

Existing evidence: the historical quotation API freeze, implemented cumulative-compilation repair at `6c050d4563ef45e7728b5bf9cddd5c7602b3176d`, and reviewed modern combined `12f3794`. The [acceptance record](docs/DECISIONS.md#d-080-add-bounded-first-order-quotation-with-callback-free-let) separates parent authority, technical audits, completed 13-file status-delta review and final follow-up review PASS `11ddbb94` and parent commit `78669c3613302249c499eba99a959f75c4c59edc`.
...
- [x] Independently review the completed [prospective cumulative-compilation repair contract](docs/plans/D080-CUMULATIVE-COMPILATION.md), preserving the historical separate-account characterization. Review `eeb4e4ba-4417-442e-b5bd-cb28ca4f4556` passed for the contract only. The approved scalar-product output fixture and full parameter tree have external baseline execution evidence; primitive ownership-layout equality has its own named coupon and exact product-owner budget oracles. Teaching receipts were refreshed through actual execution. That review established specifications only; baseline fixture evidence is not budget-repair evidence or acceptance. The subsequent implementation-stage repair now has numeric planning, cumulative coupons, literal exact/one-below budgets, the full associated differential fixture, and a production-path probe with mutation controls. Bounded implementation/layout review `6e4750c9` passed and the repair was committed at `6c050d4563ef45e7728b5bf9cddd5c7602b3176d`, then combined with partial modernization at `e56981a536e12d01be552a8a94f17ea077b61fbe`. At that implementation-stage handoff, independent combined review and remaining integration gates were pending; it changed no acceptance boxes. Subsequent source/guard PASS `b4c2d0e8`, committed-integration PASS `ddb3d314`, and decision-specific PASS `98a7de79-1e4f-4539-aa61-80cf332a2ed7` close those bounded technical gates at `12f3794`.
- [x] Review the existing implementation against that schedule. Decision-specific PASS `98a7de79-1e4f-4539-aa61-80cf332a2ed7` maps every frozen obligation to source, fixtures and retained evidence; no technical gap remains in the bounded scope.
- [x] Run forged/escaped-path compile failures, used/unused lets, shadowing, saturation, preflight, and independent primal/JVP/reverse coordinate comparisons. Actual modern committed-checkout `tests-all.log` and `D080-autodiff.log` in `/tmp/markovian-proposals-committed-validation/` pass, including exact/one-below budgets, 72 JVP/432 seed-pairing cases and ten compiled runtime mutants. Bound archive-consumer executions remain attributed to `/tmp/markovian-selector-repair/`, not new status-edit runs.
- [x] Obtain fresh independent contract and implementation review with current test evidence. Contract PASS `eeb4e4ba`, source/guard PASS `b4c2d0e8`, committed-integration PASS `ddb3d314` and the decision-specific audit above are distinct reviews.
- [x] Parent authorizes and records only bounded, unreleased acceptance in this separate status edit. Keep arbitrary-Haskell and higher-order autodiff excluded.
- [x] Obtain separate independent status-delta review: `57601418` reviewed the 13-file delta and returned PASS with P2. The changelog clarifications address P2; the technical audit did not supply this review.
- [x] Parent committed the status delta at `78669c3613302249c499eba99a959f75c4c59edc` after final independent review PASS `11ddbb94` and follow-up validation.

**Historical freeze validation (before acceptance):** D-080 freeze validation used `/home/josephcox/dev/Markovian-proposals`, branch `feat/deployment-scoped-proposals`, with base HEAD `c86b4e0241debe0a9ea51b6e9f962d89ea8293df` plus the uncommitted `docs/DECISIONS.md` and `TODO.md` documentation delta. This is current-worktree evidence, not evidence for a new immutable revision or for `v2026.9.3.0`. GHC 9.8.4 / Cabal 3.16.1.0 compiled the unchanged autodiff source and ran its focused suite. The package and boundary commands ran after the decision edit; subsequent TODO edits record those results only. Final documentation checks cover both changed files.
...
**Historical combined-modernization handoff (superseded by the completion below):** Project gates use GHC 9.14.1 and
...
**Current completion — 2026-09-08:** Modernization is locally complete at reviewed `12f3794942485b5e525def6ee470a4ea22a9f13e`, with GHC 9.14.1 / Cabal 3.18.1.0 and modern base/bytestring bounds. Combined source/guard PASS `b4c2d0e8`, committed-integration PASS `ddb3d314` and the parent source/evidence hash audit close the historical handoff's pending gates. The isolated official HLS recipe/selector/full-ABI guard and its upstream integrated-plugin limitations remain; ancillary GHC 9.8.4 is not project compatibility. Exact execution and reuse attribution is in the [D079/D080 evidence record](docs/DECISIONS.md#d-079d-080-acceptance-evidence-and-status-edit-boundary). Nothing is released; this status edit changes neither toolchain nor HLS acceptance scope.

**Status-delta validation — 2026-09-08:** `/tmp/d079-d080-status-recovered/` retains the clean `12f3794` before inventory, complete after inventory, runtime/frozen-plan/API/released-record/primary preservation guards, current-file hashes, new-file-inclusive patch, focused status/link/capability/helper/book/receipt/metadata checks and 16 local source archives with every changed-file byte membership. Commands and exit results are in `commands.jsonl` and `results.jsonl`: 37 capability tests (three new status/unreleased/projection regressions), 169 helper tests, capability/release-policy/metadata/topology checks, the 77-fence/52-file/422-math-block book check, 89 local links, and all 16 archive validators passed. The initial expected pre-edit status-test failure and two corrected external validation-harness assumptions (GitHub versus mdBook heading slugs and Cabal-normalized archive modes) remain in their original logs; no production guard was weakened. Teaching source binding and all seven outputs remain byte-identical; current receipt verification passed, so no teaching rerun or hash refresh was needed or performed. These are uncommitted worktree checks, not release or hardware evidence. Separate independent review `57601418` subsequently passed for this 13-file delta with one P2 presentation note, now addressed by dated changelog clarifications above preserved historical entries. Final follow-up review `11ddbb94` subsequently passed and parent committed the acceptance record at `78669c3613302249c499eba99a959f75c4c59edc`; the preceding execution attribution remains historical.

#### R5 — D-081 immutable host-F64 affine views

Existing evidence: [transpose reverse-equivalence repair](docs/evidence/D081-TRANSPOSE-REVERSE-EQUIVALENCE.md), not a wider affine-view implementation. Read the [reviewed r4 materialization addendum](docs/plans/D081-MATERIALIZATION-ADDENDUM.md) with the unchanged [full v3 normative plan](docs/plans/D081-AFFINE-VIEWS.md). The [durable evidence bundle](docs/evidence/D081-MATERIALIZATION/README.md) preserves exact source/review/manifest bindings, all proof summands, operative fixtures and the parent's historical-table correction. Only the addendum's named resource/producer/failure/fixture seams supersede canonical31bf; external model/demand evidence is not whole-runtime or physical-allocation validation.

- [x] Complete independent full v3 prospective-design review: PASS `5ba479ba-964e-4da4-8f95-51831227309b`. Parent approved source SHA-256 `fba9460a7201b59e292fee957957335cbd5c5d01411778fa0e37b32243658e74` after a separate 63-file bundle hash audit. This freezes the exact API/roles, ownership, admitted maps, ledgers, failure order, registry/publication protocol, lifetime A and test schedule; it does not accept D-081.
- [x] Record the frozen normative repository plan and qualify current lifetime claims without executable Haskell changes. Do not expose or snapshot the future Affine module yet.
- [x] Complete independent r4 prospective-design review: DESIGN PASS `8992df58-e052-4b23-9fd9-345febebf9d2`.
- [x] Complete focused preparation/documentation review and pre-closeout identity audit: documentation PASS `7f1a1fa6-ec54-4b26-a642-e33011ab9b74` on the 25-file patch. Parent adopts the r4 contract under `/tmp/d081-r4-freeze-closeout-authority.md`; the [freeze record](docs/evidence/D081-MATERIALIZATION/FREEZE-RECORD.md) preserves exact attribution. This does not accept D-081 or claim a commit.
- [ ] Complete final review of this narrow status closeout, then parent byte audit/staging/commit. Preserve all prior campaigns and failures without reattributing their executions.
- [ ] Obtain separate parent runtime-continuation authorization for the adopted contract. Preserve held417337 and its 12-file unstaged implementation delta; historically executed a650c2e0 and scratch-r4 are distinct. Do not apply candidate-r4 during this documentation stage. Later implement only the reviewed contract across all legacy/new registry and actual-put publication paths.
- [ ] Execute the full plan's runtime, exact/one-below, independent base-coordinate, prefix-credit, signed/empty/offset/domain, no-force mutation, synchronous/async ownership/fault, all-index opacity, installed-unit and archive-consumer schedule. Permute/reverse/slice transformations are admitted; arbitrary map composition is not. The scratch executions do not discharge these gates.
