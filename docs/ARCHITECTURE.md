...
Normalized stochastic matrices have no total trace. Feedback has four accepted checked exact fragments; the strict-discount value fragment remains unreleased. Delayed feedback has an explicit seed state and one-tick body and executes for a bounded horizon. Proper first-exit feedback uses coproduct routing `X+U -> Y+U`, checks that every represented internal state reaches an exit, solves `H=C+DH` exactly, and validates output normalization. Timed feedback keeps reward, duration, and output joint and therefore requires a nilpotent continuation block. Strict-discount value feedback accepts normalized joint reward-and-route events, solves the internal `A` and `K` right-hand sides together, and validates all four affine equations literally. It returns no evaluator or normalized exit channel. D-078 is `Accepted` only for the original coefficients and four exact equations, not EL-04's separate event-reward JVP contract or probability derivatives.

Opaque nominal witnesses and affine coefficient observations retain semantic loop ownership and endpoint types. Limits preflight represented dimensions, checked combined cardinalities, cells, events or outcomes, and graph work. Affine feedback uses row-major channel extraction and conservatively preflights every extraction, observation, reward, and event-target aggregation visit; it does not repeatedly call labelled linear matrix lookup. First-exit reachability uses a reverse queue. One finite matrix-power sequence supplies nilpotence and transience evidence.
...
The unreleased `Markovian.Continuous.Kernel.JointAffine.Exact` implements D-079's frozen left-successor substitution in the existing continuous package. Its full canonical declaration table includes zero rows; materialization uses a nonzero view of those same sources. Gate B supersedes retained-only Gate A renaming: membership, collisions, complete scope coverage, and work use all declarations. Substitution has one count-only combined preflight and rational ledger, complete all-shared-right or all-fresh-right requests, and private witnesses capturing the actual two operands and their admitted plan. Sharing requires equal Uniform intervals; freshness requires numeric disjointness from both complete manifests. The left successor enters both right coordinates; the left reward is admitted but not accumulated. The nominal sealed result retains full result sources, original-name reservations without invented laws, and complete rational history. It exposes no underlying kernel, local owner, callback, or chaining operation. Real-Borel materialization and extrema are separately budgeted but admit and seed their meters with the substitution's historical bit maximum, including discarded inputs and canceled intermediates. D-079 is Accepted only within the bounded, unreleased scope in its [separate acceptance record](DECISIONS.md#d-079-add-exact-joint-affine-continuous-kernels); the [frozen contract](plans/D079-LEFT-SUCCESSOR-SUBSTITUTION.md) defines the exact signatures, schedules, and evidence boundary. This is neither temporal reward accumulation nor a general composition/category or device claim.
...
Release `2026.9.3.0` is the first public API. From `2026.9.15.1`, calendar versions identify releases without a PVP compatibility promise. The amended D-062 requires exact sibling release bounds, explicit migration notes, evidence-backed `tested-with` fields, package-specific metadata, complete Haddock, and exposed-module snapshots.
...
D-080 is Accepted only for the bounded, unreleased quotation and cumulative-compilation scope in its [separate acceptance record](DECISIONS.md#d-080-add-bounded-first-order-quotation-with-callback-free-let). Its evidence covers opaque nominal paths and explicit exact-polynomial quotations. Generative tokens separate equal-shaped lexical scopes. `letQuote` stores a body term instead of a callback. Quotation compilation continues its syntax ledger with charged numeric metadata planning and the frozen cumulative logical reconstruction coupons. Traversal, allocation, and runtime reservations precede a strict private witness, then the builder and unchanged exact compiler with derived capacities and caller rational bits. The compilation report contains the complete quotation reservation and actual compiler report. Syntax-only preflight/lowering and direct primal/JVP schedules remain unchanged, including their historical accessor traversals. Compiler coupons count logical reconstruction, not physical allocations or peak residency. Direct exact primal and JVP recursion stays independent from reverse lowering. This API is not Haskell quotation, Template Haskell, a plugin, tracing, or nested differentiation.
