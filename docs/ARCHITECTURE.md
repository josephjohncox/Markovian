...
D-081 is `Accepted` within its bounded, unreleased scope. `Markovian.Tensor.Affine` retains the actual original base and owner for admitted maps and base-coordinate pullbacks. The [acceptance record](evidence/D081-AFFINE-IMPLEMENTATION.md) binds its frozen contracts, logical-resource proof, implementation evidence, and accepted historical index exception. This adds no arbitrary map composition, broadcasting, mutation, pointer, sparse, pinned, foreign, or device API.
