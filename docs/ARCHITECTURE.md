...
D-080 remains proposed. Its evidence adds opaque nominal paths and explicit exact-polynomial quotations. Generative tokens separate equal-shaped lexical scopes. `letQuote` stores a body term instead of a callback. One cumulative ledger bounds traversal, coordinates, machine extent, allocation, lowering, compilation, arithmetic work, and rational size. Preflight charges before descent and constructs no target `Program` on failure. Direct exact primal and JVP recursion stays independent from reverse lowering. This API is not Haskell quotation, Template Haskell, a plugin, tracing, or nested differentiation.
