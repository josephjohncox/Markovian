...
Normalized stochastic matrices have no total trace. Feedback has three accepted checked exact fragments and one proposed strict-discount value fragment. Delayed feedback has an explicit seed state and one-tick body and executes for a bounded horizon. Proper first-exit feedback uses coproduct routing `X+U -> Y+U`, checks that every represented internal state reaches an exit, solves `H=C+DH` exactly, and validates output normalization. Timed feedback keeps reward, duration, and output joint and therefore requires a nilpotent continuation block. Strict-discount value feedback accepts normalized joint reward-and-route events, solves the internal `A` and `K` right-hand sides together, and validates all four affine equations literally. It returns no evaluator or normalized exit channel. D-078 remains `Proposed`.
