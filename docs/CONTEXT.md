...
D-053 remains `Accepted`. D-054 through D-060 remain `Proposed`, and their slices are blocked at integration acceptance even where focused local fixtures pass. CI now names all four packages, all four inventory benchmarks, and the game, reverse-program, and exact-support boundaries; the remaining current-revision evidence above is required before any status promotion.
