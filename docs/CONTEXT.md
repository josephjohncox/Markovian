...
The core library exposes validated floating and exact probability, reward, objective, kernel, MRP, MDP, policy, sampling, trace, compilation, tabular-learning, exact POMDP, categorical IR, dense CPU, and bounded inventory-benchmark modules. Policy-free exact compilation stores every available action and joint outcome. A separate operation closes a compiled model under a policy. Exact interpreters provide bounded expectation, trace enumeration, finite-horizon policy evaluation, contraction Bellman policy evaluation, bounded value iteration, deterministic greedy extraction, rational policy iteration, filtering, and bounded belief planning. The inventory benchmark uses finite-horizon backward induction for a synthetic serial fixture, compares duplicate-free period-specific base-stock grids, and reports only validated primary-versus-widened bounds.

The sampled and tabular layers share one explicit-generator joint MDP step. They provide pure TD(0), SARSA, Expected SARSA, and Q-learning updates. Each method has a bounded resumable episodic runner. The neural package provides checked dense networks, a small typed parametric reverse interpreter, stable categorical operations, approximate entropy, cross entropy, KL divergence, mutual information, and logit gradients, linear REINFORCE and actor-critic updates, FIFO replay, target-network synchronization, and one standard or Double-DQN batch update. It has no tensor, general-autodiff, device, environment-runner, or complete-trainer dependency.
...
The root test aggregator includes focused `ExactControl`, `TabularLearning`, `PushPullExact`, and `InventoryBenchmark` modules. Their deterministic fixtures cover exact compilation and control, exact state-payoff push-pull, checked inventory provenance and bounds, tabular targets, terminal timing, seeded behavior, bounded loops, and resumable runs. The neural test aggregator includes `ParametricReverse` plus dense finite differences, categorical information and policy gradients, REINFORCE, actor-critic, replay, target networks, and DQN fixtures.

On the current worktree, GHC 9.8.4 passed the focused root and neural suites after the integration repairs. The root fixtures include synthetic serial timing, conservation, supplier delay, feasibility, truncation, reachable closure, terminal behavior, independent oracle, base-stock, support order, checked widening, constructor rejections, unit discount, report golden, and benchmark configuration checks. The reverse fixtures include cotangent-module and VJP laws, identity and associativity checks, exact diagonals, nonlinear composition and diagonal finite differences, and composed failure propagation. The benchmark completed one excluded warm-up and twenty measured samples with stable exact semantic output. The warning-free full Haddock build, focused HLint and Fourmolu checks, package metadata checks, refinement-role check, and book check passed. All three source tarballs were generated and include the inventory benchmark and golden report. Unpacked source-distribution builds, lower-bound, GHC 9.4.8, hosted, enabled GPU, and other platform checks remain to be rerun for this repaired worktree.
...
| INV-INVENTORY | Exact inventory values apply only to the conditional bounded-demand model; reachable successors are not clamped, truncation mass is not a value-error bound, and unrelated or non-widened solutions cannot produce stable evidence. | Opaque fixture and solution, support closure, checked widening witness, and report tests |
...
S7.1 information, categorical-learning, polarity, and game-semantic foundations are implemented. The bounded S7.3 push-pull slice, S7.2 synthetic bounded serial fixture, and small S7.4 typed parametric reverse interpreter are implemented. The reverse interpreter is confined to `markovian-neural`; it uses explicit parameter products and cotangent-module witnesses, with no optimizer or autodiff semantics. A wider reverse syntax remains blocked on an owned primitive set, primal storage policy, backend arithmetic, and equality relation. Finite interaction protocols remain blocked until one protocol defines ownership, legal and terminal histories, strategy composition, and observational equality. The named serial reproduction remains blocked until its complete primary-source equations and timing are verified. The multi-retailer balance relaxation and fixed-batch fixture remain deferred. The next coherent task is S7.3's exact circuit cost interpretation and deterministic proof-carrying rewrites. Arbitrary cyclic graphs, feedback, continuous-time open Markov black-boxing, unrestricted MDP black-boxing, tensor frameworks, general autodiff, devices, and production trainers remain out of scope.
