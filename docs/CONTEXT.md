...
The separate `Pages` workflow is configured to publish the checked book from `main`. The project path is `/Markovian/`, and the configured URL is <https://josephjohncox.github.io/Markovian/>. No successful hosted deployment is recorded for this worktree. Publication remains blocked until the repository enables Pages with GitHub Actions and a hosted run proves the URL.
