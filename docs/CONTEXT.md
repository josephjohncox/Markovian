...
Markovian is an early Haskell prototype. One Cabal library exposes the unchanged legacy `Markovian` and `QLearning` modules plus the additive `Markovian.*` core modules.
...
D-022 authorizes one additive Foundation Kickoff slice. New modules define opaque floating probabilities, weights, finite distributions, rewards, one-layer kernels, policies, and one-step MRP and MDP interfaces. Action IDs and stochastic transition outcomes are separate types. Terminal state inspection returns a terminal payoff without running a transition kernel. Finite normalization removes every mass that rounds to zero before it exposes support.

`test/Main.hs` contains deterministic core contract tests. `Markovian.cabal` exposes the new modules and lists all durable documents in `extra-doc-files`.
...
The package check, `-Werror` build, seven tests, warning-free Haddock generation, and source-distribution creation pass locally on GHC 9.4.8 and 9.8.4. The pinned formatter installation and checks pass on GHC 9.8.4. FK.4 is `DONE`. D-024 defines `.github/workflows/ci.yml`, but P0.1 remains `ACTIVE` until the hosted workflow has a durable successful run.
...
| K-010 | `test/Main.hs` | Resolved: deterministic core contract tests replaced the placeholder and pass under GHC 9.8.4. Legacy characterization tests remain P0.2 work. |
| K-011 | `Markovian.cabal` | Partly resolved: the package now supports `base` 4.17 through 4.19 and builds on GHC 9.8.4. Component dependencies remain duplicated and need direct-import review. |
| K-012 | `Markovian.cabal` | Resolved for the current package check: synopsis, description, category, repository metadata, distributed documents, and source-distribution creation have command evidence. |
...
3. Complete P0.1, the active task in `TODO.md`. Commit or push the pinned workflow, record a successful durable CI URL, and preserve the FK.4 gates.
