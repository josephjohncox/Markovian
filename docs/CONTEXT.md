...
On the current worktree, GHC 9.8.4 built and tested all three packages through `cabal.project.ci`; the CUDA package used its disabled contract. All packages also passed preferred-oldest resolution, `cabal check`, HLint, Fourmolu, cabal-fmt, warning-free Haddock, and independent unpacked source-distribution builds and tests. `scripts/check-book`, all 323-equation MathJax browser startup, `git diff --check`, and the documentation STE linter passed. Separate mathematical, pedagogical, and implementation-grounding reviews checked the polarity chapter and foundation narrative. Their blocking findings on Bayesian support, cotangent duals, Bellman hypotheses, terminology, and chapter prerequisites were corrected. These local checks do not establish GHC 9.4.8, enabled-GPU, hosted, or other-platform support for this revision.
