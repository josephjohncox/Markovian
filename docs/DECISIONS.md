...
Preflight covers input cells, output cells, source circuit work, cache entries, executor work and rational size before table allocation. Cache construction must retain the ordered semantic validation evidence, including discarded rational maxima and first-failure boundaries; a final matrix and aggregate work count alone are insufficient. Primitive callbacks must be pure, deterministic and stable under the explicit interpreter identity; arbitrary closure identity and effectful or time-varying callbacks are excluded. Changed semantic limits miss the key under this first contract, even when a less restrictive trace could theoretically be reused. See the [frozen resource-admission policy](plans/EL-06-RESOURCE-ADMISSION.md) for the 100/1/10 case and error relation. This selects a contract, not a cache implementation or completed EL-06 experiment.
