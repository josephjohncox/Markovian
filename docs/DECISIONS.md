...
**Decision (amended 2026-09-15):** Use coordinated UTC CalVer `YYYY.M.D.N` to identify releases. Calendar components do not promise PVP compatibility. Pin public sibling dependencies to the exact coordinated release, such as `==2026.9.15.1`, and document incompatible changes in migration notes. Every exposed declaration requires Haddock documentation, and each package has a reviewed exposed-module snapshot and explicit tested compiler.

**Current boundary:** The original `2026.9.3.0` release used PVP caret bounds. The `2026.9.15.1` release keeps calendar numbering and adds a constructor to the public tensor error type, so the old same-month PVP compatibility promise cannot apply. Existing downstream `^>=2026.9.3.0` bounds must be reviewed; exact bounds in the new package set cannot change those older constraints. The same-day sequence may skip unused values. Each `source-repository this` section identifies the exact release tag and package subdirectory. `release/published-releases.json` prevents rebuilding a published version from another source revision.

**Consequences:** Upgrade the coordinated package set together and check migration notes. Every changed source release needs a new version. Hackage metadata revisions cannot change package code. Compiler support is explicit rather than inferred between tested endpoints.
...
### D-084: Add a bounded reference DQN trainer

**Status:** Accepted

**Bounded, unreleased acceptance — 2026-09-14:** Accept the reference trainer at `03493e83bd842d2e9c0b41d0c0de4ab4380e4dc3` in `markovian-neural-bridge`. The [contract](plans/D084-DQN-TRAINER.md) fixes fuel, replay/checkpoint ownership, generator advancement, failure recovery, and publication. Independent O0/O2 review checked seeded split runs, partial sampling failure, bootstrap timing, all six preflight limits, atomic updates, constructor forcing, and nine constructor-export mutation controls. The benchmark records twenty raw timings and a separate semantic receipt. The bridge owns the numerical/sampling dependencies; the neural package remains independent of the root. This accepts a pure reference trainer, without production or convergence claims.
...
**Rationale:** Replay, target synchronization, and the checked DQN update need an explicit temporal composition. A fuel-bounded trainer can serve as a deterministic integration reference without a production-training claim.

**Consequences:** This implementation adds no production trainer, convergence result, environment framework, tensor runtime, device execution, distributed replay, asynchronous actor, hidden thread, global RNG, or scalability claim. Wall-clock measurements remain empirical and separate from semantic step reports.
...
**Bounded, unreleased acceptance — 2026-09-14:** Accept the retained local-table consumer at `03493e83bd842d2e9c0b41d0c0de4ab4380e4dc3` under the [D-085 contract](plans/D085-CIRCUIT-CACHE.md). Independent O0/O2 review checked every circuit constructor, complete keys, ordered cumulative admission, strict publication, ownership, and a reversed-replay mutation. A separate oracle passed 45,144 admission comparisons. [Raw measurements](evidence/D085-CIRCUIT-CACHE.csv) compare uncached, cold, and warm execution at both optimization levels. The tested workload improves time/allocation while increasing logical executor operations; acceptance makes no universal speedup claim. The global frontier phase retains its existing resource contract.
...
Construction and all subsequent occurrences within a request share one cumulative semantic account and one separate executor account. Construction is charged once as that occurrence's source execution, not again as first use; each later hit incurs the full source trace again plus its executor work. Only complete successful tables and validated traces may be retained; failure leaves no partial entry and arbitrary failures are not cached. Semantic limits are validated on every request and source checks replayed at their original positions. The first implementation admits only proof-carrying exact table primitives under `ExactTableInterpreterV1`; ordered validation follows its closed checked source algorithm. Arbitrary callbacks remain available in the original interpreter and cannot inhabit this cache. Hidden state, IO, opaque unaccounted costs, unrepresented limit-dependent behavior, arbitrary closure identity and equality inferred from labels alone are excluded.
