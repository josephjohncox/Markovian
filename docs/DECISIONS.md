...
The separate decision-specific audit reports are `d079-decision-acceptance-audit.md` (PASS `b4c30c96-f3ef-40ba-8ff0-bdf038e9e4df`) and `d080-decision-acceptance-audit.md` (PASS `98a7de79-1e4f-4539-aa61-80cf332a2ed7`) under session artifacts `/home/josephcox/.pi/agent/sessions/--home-josephcox-dev-Markovian--/subagent-artifacts/outputs/c68a0ab8-4c3c-411e-8496-63872e4ce00b/`. Each contains a complete frozen-obligation matrix and no identified technical gap. Parent authorization is `/tmp/markovian-d079-d080-status-authority.md`, continued by `/tmp/markovian-status-recovery-direction.md`; these are acceptance authority, not test executions. Separate independent status-delta review `57601418` reviewed the 13-file delta and returned PASS with one P2 presentation note; it is not either technical audit. Its report is `d079-d080-status-independent-review.md` under session artifacts `/home/josephcox/.pi/agent/sessions/--home-josephcox-dev-Markovian--/subagent-artifacts/outputs/61401261-3292-4687-b1f3-6488b003ee2d/`. Dated continuous/autodiff changelog clarifications address P2 while preserving the original implementation-stage entries and all released sections. Final status-delta review `11ddbb94` passed after follow-up validation; parent committed the bounded D079/D080 acceptance record at `78669c3613302249c499eba99a959f75c4c59edc`. This closes that historical status-edit handoff only.

Combined source/guard review `b4c2d0e8` and committed-integration PASS `ddb3d314` bind `12f3794`. The parent audit `/tmp/markovian-modern-parent-final-owxkm3hw/result.json` checks 579 source blobs/modes and 8,637 retained evidence files. `/tmp/markovian-proposals-committed-validation/` records actual modern committed-checkout all-package tests, D079/D080 boundaries and seven teaching-fixture executions. Its `reuse-proof.json` binds the earlier `/tmp/markovian-selector-repair/` campaign's current-byte archive consumers, lower-bound tests, benchmarks and warning-enabled documentation (16 public libraries plus private `legacy-finite-ir`) without relabeling them as new executions. D079's unchanged semantic/probe mutation controls retain their original `/tmp/d079-correction-validation-NnuNuzNF` attribution. D080's source repair is `6c050d4563ef45e7728b5bf9cddd5c7602b3176d`, integrated at `e56981a536e12d01be552a8a94f17ea077b61fbe` before final `12f3794`.

The modernization is locally complete at `12f3794` within the approved GHC 9.14.1 / Cabal 3.18.1.0 / base 4.22 / bytestring 0.12.2 baseline. GHC 9.8.4 retains only its ancillary HLint/cabal-fmt construction role. The isolated official-source HLS recipe, selector/seal/full-ABI guard and upstream integrated-plugin limits remain unchanged; construction, reused construction, ABI checks and actual LSP operation are distinct evidence. No user-global tooling, unavailable HLS plugin, fresh hosted CI, hardware execution or cryptographic attestation is claimed. D077 hardware evidence binds only its previously tested `3e850085…`, not this source.

D-077 and original D-078 remain Accepted; EL-03, EL-04, EL-05 and D-081–D-085 remain Proposed. No capability row is added merely to accept D079/D080. All availability remains unreleased for this work; 16 package versions remain `2026.9.3.0`, with no change to immutable released membership, assets, topology or the D061–D076 source release-policy guard. Focused status-delta validation, complete before/after hashes and new-file-inclusive patch are retained in `/tmp/d079-d080-status-recovered/`; these local validation archives are not release artifacts.
...
Acceptance is supported by independent exact-budget, all-coordinate associated-fixture, production-path mutation, opacity, modern-compiler, warning-enabled documentation, and standalone-archive evidence bound to the reviewed source. See the [shared acceptance evidence and status-edit boundary](#d-079d-080-acceptance-evidence-and-status-edit-boundary). Historical freezes and executions retain their original meanings. The [frozen cumulative-compilation plan](plans/D080-CUMULATIVE-COMPILATION.md) remains byte-identical, including its prospective header; this decision records current status. This is design/current-work acceptance, not release or deployment approval.
...
**Current design freeze:** Parent approved the exact [full v3 affine-view contract](plans/D081-AFFINE-VIEWS.md), source SHA-256 `fba9460a7201b59e292fee957957335cbd5c5d01411778fa0e37b32243658e74`, after independent prospective-design PASS `5ba479ba-964e-4da4-8f95-51831227309b` and an independent 63-file bundle hash audit. Authority `/tmp/d081-repository-freeze-authority.md` records design approval, not runtime authorization or decision acceptance. The plan freezes exact declarations/roles, actual-original-base ownership, admitted signed maps, separate planning/runtime ledgers, numerical coupons, ordered diagnostics, universal legacy/new registry-spine and real-publication ownership obligations, temporal lifetime A, and the future test schedule. Current public lifetime claims are qualified without changing executable Haskell. The fully forced registry and Affine API are not implemented; neither Cabal exposure nor the current module snapshot claims them. Model/typecheck/source evidence is not runtime evidence. Physical repository-freeze validation and independent review, then parent byte audit/commit, remain separate from later implementation authorization and eventual acceptance. D-082 remains blocked; D-083 placement and all other statuses remain unchanged.

#### Historical original proposal and transpose-only prerequisite evidence

The following proposal and evidence record is preserved verbatim. The full v3 plan above now controls future implementation; it does not reinterpret the existing transpose tests as signed-map or base-pullback evidence.
...
View creation allocates no payload. Materialization and each primitive preflight the complete destination and work before allocation. Reverse rules for the proposed affine-view operations create fresh cotangents and must agree with a materialize-first reference under the frozen host-F64 comparison policy. The existing closed addition tape retains its accepted immutable seed-sharing behavior and is not redefined by this proposal. Semantic `TensorOwner`, physical `StorageId`, region, shape, and view-map evidence remain distinct and nominal.
...
Tabulation is interpreter-relative. The selected policy is **source-semantic admission**, not actual-executor-work admission. It preserves the selected pure exact interpreter's literal denotation, semantic admission, and semantic failure precedence by replaying a retained, validated source charge/validation trace under the caller's cumulative semantic limits. A hit cannot replace a 100-unit source charge with a one-unit admission charge. Executor work, cache construction, storage, lookup, and replay cost are separately bounded and reported; their cache-specific failures are not claimed identical to uncached semantic failures. The uncached interpreter remains the reference path.

Infrastructure preflight bounds cache entries, cells, trace storage and executor work first; infrastructure rejection makes no source-result claim. Then source structural preflight bounds its input/output cells and source plan before table allocation. Semantic work and rational validation follow the reference source's ordered checks, including discarded intermediates; moving an input or intermediate rational check ahead of an earlier source failure is not permitted. Construction must retain ordered semantic validation evidence, not only a final matrix, aggregate charge or discarded-rational maximum. Trace allocation and replay are themselves bounded executor work. Replay cannot be treated as free.

Construction and all subsequent occurrences within a request share one cumulative semantic account and one separate executor account. Construction is charged once as that occurrence's source execution, not again as first use; each later hit incurs the full source trace again plus its executor work. Only complete successful tables and validated traces may be retained; failure leaves no partial entry and arbitrary failures are not cached. Semantic limits are validated on every request and source checks replayed at their original positions. Primitive callbacks must be pure, deterministic, terminating and stable under the explicit interpreter identity and limits, with enough checked evidence to replay their source contract. Hidden state, IO, opaque unaccounted costs, unrepresented limit-dependent behavior, arbitrary closure identity and equality inferred from labels alone are excluded.

Changed semantic limits miss the key under this first contract, even when a less restrictive trace could theoretically be reused. A real successful 100-budget entry therefore misses at ten; the hypothetical one-unit hit in the lesson is an accounting-path assumption, not evidence that such an entry exists. With sufficient infrastructure allowance, both paths reject at source unit 11 under semantic budget ten; both may succeed at 100 while separately reporting different executor reservations. An executor budget of ten may instead distinguish their infrastructure admission. See the [frozen resource-admission policy](plans/EL-06-RESOURCE-ADMISSION.md) and [checked EL-06 lesson](book/src/resource-admission.md). EL-06 implements an accounting model and a real checked-bind association counterexample, **not** this cache. Its one-unit hit cost is neither measured nor demonstrated for real replay. Source association remains part of admission; equal admitted denotation does not authorize silent reassociation or an unrestricted Monad/optimizer claim.
