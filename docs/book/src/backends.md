...
Each finite F64 input word first denotes an exact dyadic rational. Exact matrix and VJP evaluation defines the fixture specification. The CPU path refines it with separate multiply-add operations in ascending `k` order. The CUDA path refines it with the committed `fma.rn.f64` PTX sequence. Neither floating path is the oracle for the other.

Successful current-process admission is not general device-correctness evidence. The repository has no complete D-077 receipt for the historical D-074 hardware run. The package does not support arbitrary tensor graphs, generic reverse-program lowering, arbitrary strides, zero-size launches, F32, stochastic nodes, devices outside the pinned profile, cross-device bit equality, or a speed claim.
