...
The script runs package checks, tests, lower-bound resolution, boundaries, benchmarks, Haddock, and the book check. Its isolated Haddock installation rejects warnings and requires one interface per package; a separate warning-suppressed pass measures declaration coverage only. It then creates each source archive twice.
