...
import Markovian.DenseExact.Internal.LegacyFiniteIR
...
testLegacyDifferential = do
    ...
    pairObject <- objectC pairSupport
    ...
    assertC "circuit CPU lowering changed source layout" (denseExactSource newDense == unitObject)
    assertC "circuit CPU lowering changed target layout" (denseExactTarget newDense == pairObject)
    assertC
        "circuit CPU lowering changed source-by-target rows"
        (fmap NonEmpty.toList (NonEmpty.toList (denseExactRows newDense)) == [[1 % 2, 0, 0, 1 % 2]])
    ...
    assertC
        "independent circuit CPU lowering changed"
        (fmap NonEmpty.toList (NonEmpty.toList (denseExactRows newDenseIndependent)) == [[1 % 4, 1 % 4, 1 % 4, 1 % 4]])
