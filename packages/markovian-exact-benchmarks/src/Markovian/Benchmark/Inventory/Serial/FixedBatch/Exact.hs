...
-- | Validate the fixed batch parameters.
...
data FixedBatchState = FixedBatchState
    ...
    -- ^ The fixed batch periods remaining.
    , fixedBatchStage1NetInventory :: !Integer
    -- ^ The fixed batch stage 1 net inventory.
    , fixedBatchStage2OnHand :: !Natural
    -- ^ The fixed batch stage 2 on hand.
    , fixedBatchPipeline :: ![Natural]
    -- ^ The fixed batch pipeline.
    }
    deriving (Eq, Ord, Show)

-- | Return the fixed batch state.
...
-- | Return the fixed batch omitted mass.
...
-- | Return the fixed batch actions.
...
-- | Return the fixed batch transition.
...
-- | Return the fixed batch outcomes.
...
-- | Return the fixed batch fixture.
...
-- | Compare the fixed batch state support.
...
-- | Compare the fixed batch state layout.
...
-- | Return the reorder grid candidates.
...
-- | Return the fixed batch policy action.
...
data FixedBatchSolution = FixedBatchSolution
    ...
    -- ^ The fixed batch solution fixture.
    , fixedBatchSolutionGrid :: !ReorderGrid
    -- ^ The fixed batch solution grid.
    , fixedBatchOracleReturn :: !Rational
    -- ^ The fixed batch oracle return.
    , fixedBatchPolicyReturn :: !Rational
    -- ^ The fixed batch policy return.
    , fixedBatchSelectedLevels :: !ReorderLevels
    -- ^ The fixed batch selected levels.
    , fixedBatchOracleExternalCapSelected :: !Bool
    -- ^ The fixed batch oracle external cap selected.
    , fixedBatchPolicyExternalCapClipped :: !Bool
    -- ^ The fixed batch policy external cap clipped.
    , fixedBatchPolicyReleaseClipped :: !Bool
    -- ^ The fixed batch policy release clipped.
    , fixedBatchGridBinds :: !Bool
    -- ^ The fixed batch grid binds.
    , fixedBatchCheckedSolverWork :: !Natural
    -- ^ The fixed batch checked solver work.
    }
    deriving (Show)
...
-- | Return the fixed batch policy regret.
...
-- | Evaluate the fixed batch policy.
...
-- | Compare the fixed batch bounds.
...
-- | Compare the fixed batch demand caps.
