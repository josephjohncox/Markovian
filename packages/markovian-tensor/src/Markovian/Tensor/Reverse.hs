...
data UnaryTape region input output = UnaryTape !(Internal.UnaryTape region input output)

type role UnaryTape nominal nominal nominal

-- | Opaque tape for one closed binary host primitive.
data BinaryTape region left right output = BinaryTape !(Internal.BinaryTape region left right output)
...
tanhWithTape session input =
    fmap (fmap (\(output, tape, report) -> (output, UnaryTape tape, report))) $
...
addWithTape session left right =
    fmap (fmap (\(output, tape, report) -> (output, BinaryTape tape, report))) $
