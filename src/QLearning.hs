...
-- QLearning module
module QLearning (QTable, qLearning, evaluateMDPQLearning) where

import Control.Monad.Random (MonadRandom, getRandomR)
import Control.Monad.State.Class (MonadState, get, modify)
import Data.Map.Strict qualified as Map
import Data.Vector qualified as V
import Markovian (Action (..), MDPF (..), MendlerAlgebraM, Process (..), mdpCoalgebra, mhyloM)
...
argmaxAction sName actions qTable =
    V.maxIndex $
        V.imap
            ( \_ action ->
                Map.findWithDefault 0 (sName, actionName action) qTable
            )
            actions
