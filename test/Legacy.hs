module Legacy (runLegacyTests) where
...
runLegacyTests :: (String -> IO () -> IO ()) -> IO ()
runLegacyTests run = do
    ...
    let finish = actionId Finish
        ...
        model =
            mdp
                ...
                (kernel (const stochasticOutcomes))
...
testExactFiniteCompilation = do
    ...
    compiledModel <-
        requireRight
            ...
            (compileExactMDP [EvalStart, EvalTerminal] [lowAction, highAction] model)
    compiled <- requireRight "compiled exact policy closure" (closeCompiledExactPolicy compiledModel selectedPolicy)
    assert "compiled initial state changed" (stateIndexValue (compiledInitialState compiledModel) == 0)
    assert "compiled state count changed" (length (compiledStates compiledModel) == 2)
    compiledLow <-
        maybe
            (failTest "compiled low action was not indexed")
            pure
            (lookupActionIndex (compiledActionIndex compiledModel) lowAction)
    assert "compiled low action index changed" (actionIndexValue compiledLow == 0)

    compiledStep <- requireRight "compiled policy step" (stepCompiledExactMRP compiled (compiledMRPInitialState compiled))
    compiledOutcomes <-
        case compiledStep of
            ...
            CompiledExactTransitionStep distribution ->
                maybe
                    ...
                    pure
                    (decodeCompiledOutcomes compiledModel distribution)
    direct <-
        requireRight
    ...
    reversedModel <-
        requireRight
            ...
            (compileExactMDP [EvalTerminal, EvalStart] [highAction, lowAction] model)
    reversed <- requireRight "reordered exact policy closure" (closeCompiledExactPolicy reversedModel selectedPolicy)
    reversedStep <- requireRight "reordered compiled policy step" (stepCompiledExactMRP reversed (compiledMRPInitialState reversed))
    reversedOutcomes <-
        case reversedStep of
            ...
            CompiledExactTransitionStep distribution ->
                maybe
                    ...
                    pure
                    (decodeCompiledOutcomes reversedModel distribution)
    ...
    compiledTerminalModel <-
        requireRight
            "terminal-only finite compilation"
            (compileExactMDP [EvalTerminal] [] terminalOnly)
    compiledTerminal <- requireRight "terminal-only policy closure" (closeCompiledExactPolicy compiledTerminalModel terminalPolicy)
    terminalStep <- requireRight "compiled terminal-only step" (stepCompiledExactMRP compiledTerminal (compiledMRPInitialState compiledTerminal))
    ...
    case compileExactMDP [EvalStart, EvalStart] [lowAction, highAction] model of
        Left (ExactMDPStateIndexError (DuplicateFiniteIndexValue EvalStart)) -> pure ()
        result -> failTest ("compiled duplicate state index was not rejected: " ++ showCompileResult result)
    case compileExactMDP [EvalStart, EvalTerminal] [lowAction, lowAction] model of
    ...
    case compileExactMDP [EvalStart, EvalTerminal] [lowAction] model of
        Left (ExactMDPUnindexedAvailableAction EvalStart unavailable) ->
    ...
    case compileExactMDP [EvalStart] [lowAction, highAction] model of
        Left (ExactMDPUnindexedSuccessor EvalStart selectedAction EvalTerminal) ->
...
testExactFiniteDynamicProgramming = do
    ...
    compiledModel <-
        requireRight
            ...
            (compileExactMDP [EvalStart, EvalTerminal] [lowAction, highAction] model)
    compiled <- requireRight "dynamic-programming policy closure" (closeCompiledExactPolicy compiledModel selectedPolicy)
    ...
    compiledLoopModel <-
        requireRight
            ...
            (compileExactMDP [EvalLoop] [onlyAction] loopModel)
    compiledLoop <- requireRight "self-loop dynamic-programming closure" (closeCompiledExactPolicy compiledLoopModel loopPolicy)
...
testExactBellmanFixedPoint = do
    ...
    compiledLoopModel <-
        requireRight
            ...
            (compileExactMDP [EvalLoop] [onlyAction] loopModel)
    compiledLoop <- requireRight "Bellman self-loop closure" (closeCompiledExactPolicy compiledLoopModel loopPolicy)
    ...
    compiledTerminalModel <-
        requireRight
            ...
            (compileExactMDP [EvalTerminal] [] terminalModel)
    compiledTerminal <- requireRight "terminal Bellman closure" (closeCompiledExactPolicy compiledTerminalModel terminalPolicy)
